DROP TABLE IF EXISTS dwd.dwd_edu_service_payment_orders;
CREATE TABLE IF NOT EXISTS dwd.dwd_edu_service_payment_orders
(
    ...
    sales_name_2             STRING COMMENT '销售姓名（不拼接）'
)
    COMMENT '教服业绩-支付订单总表'
...
CREATE TEMPORARY TABLE order_data_all as
...
SELECT
    c.goods_name                                                              as order_period_name
     ...
     , a.pay_time                                                                                                                                                  --支付时间
     , a.resource_id                                                                                                                                               --商品id
     , IF(a.resource_id RLIKE '^course', '糖豆舞蹈私教课-月卡', b.goods_name) as goods_name                                                                        --商品
    ...
    END                                                                       AS pay_way                                                                           --支付方式
     ...
     , NVL(CONCAT(NVL(department_cte.dept_name,''), NVL(department_cte.grp_name,''), '_', NVL(department_cte.sales_name,''), '(',
                     a.owner_id, ')', '_',
                     REPLACE(NVL(department_cte.corp_name,''), '小糖乐学', '')), '未分配')  as sales_name                                                           --订单归属人
...
--                        , user_group
                  ...
                  WHERE order_state IN (1, 10, 11) --订单成功的
                    AND xiaoe_order_type != '开放API导入订单'
                    AND price > 0
                    -- AND pay_time >= '2025-03-17 00:00:00'
                    -- AND pay_time <= '2025-04-30 23:59:59'
                    and xe_app_id = 'appcafhwq5q8671' --太极
                    AND  resource_type in ('6', '100006', '8', '100008')
              ) a
     ) a
...
--                         and a.owner_class = f.xiaoe_order_resource_id
         ...
         LEFT JOIN (select id,platform from ods.ods_place_h5) h5
                   on a.h5_id = h5.id
;
INSERT OVERWRITE TABLE dwd.dwd_edu_service_payment_orders
SELECT
    order_period_name
    ,member_id
    ,h5_id
