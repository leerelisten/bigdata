SET mapred.job.name="c_app_course_period_tencentsucai_dashboard#${datebuf}";
USE app;
CREATE EXTERNAL TABLE IF NOT EXISTS app.c_app_course_period_tencentsucai_dashboard
(
    ...
    COMMENT '培训主题数仓-腾讯素材期次报表'
    PARTITIONED BY (dt STRING)
    ROW FORMAT DELIMITED
        FIELDS TERMINATED BY '\t'
    STORED AS TEXTFILE
    LOCATION '/dw/app/c_app_course_period_tencentsucai_dashboard';
...
WITH cost AS
   ...
   -- cost_id2是为了是处理手工导入消耗，头条信息流这里不涉及，不需要
   , user_num AS
    (SELECT cost_id
          , ad_id
          , sucaiid
   ...
   , mid AS
    ( -- 按期次+投放属性聚合用户
        SELECT  goods_name
             ...
             , CONCAT('id_', sucaiid) 															  AS sucai_id
             , sucai_name
             , sucai_type
...
INSERT
    OVERWRITE
    TABLE
    app.c_app_course_period_tencentsucai_dashboard
    PARTITION
(
    dt = '${datebuf}'
)
...
DFS -touchz /dw/app/c_app_course_period_tencentsucai_dashboard/dt=${datebuf}/_SUCCESS;