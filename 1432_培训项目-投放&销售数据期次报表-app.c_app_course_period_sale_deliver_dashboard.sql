...
-- 8_期次x渠道x版位X部门X组
-- 9_期次x渠道x版位X部门X组X销售
-- 10_期次x渠道x版位X部门X组X销售X账户

-- 已按照期次报表兼容人工导入的消耗


SET mapred.job.name="c_app_course_period_sale_deliver_dashboard#${datebuf}";
USE app;
CREATE TABLE IF NOT EXISTS app.c_app_course_period_sale_deliver_dashboard
(
    ...
    cost_id             string COMMENT '账户',               --20241225新增到账户维度
    ...
    wx_active_num       int COMMENT '主动加微例子数(个)',--调整位置20241126
    collect_active_num  int COMMENT '主动填问卷例子数(个)',--调整位置20241126
    payment_num         int COMMENT '支付成功例子数(个)',    --新增20241126
    interest_num        int COMMENT '太极兴趣度例子数(个)',--新增20241126
    influence_num       int COMMENT '太极紧迫度例子数(个)',--新增20241126
    sex_num             int COMMENT '太极女性例子数(个)',--新增20241126
    age_num             int COMMENT '太极50岁以上例子数(个)',--新增20241126
    ...
    COMMENT '培训主题数仓-投放&销售数据期次报表(添加渠道、版位)'
    PARTITIONED BY (dt STRING)
    ROW FORMAT DELIMITED
        FIELDS TERMINATED BY '\001'
    STORED AS TEXTFILE
    LOCATION '/dw/app/c_app_course_period_sale_deliver_dashboard';
...
--        AND a.goods_name NOT LIKE '%测试%'
   ...
   , mid AS
    ( -- 按期次+投放属性聚合用户
        SELECT TO_DATE(created_at)    AS d_date
             ...
             , sales_name
             , sales_sop_type
             , CONCAT('id_', cost_id) AS cost_id            --新增到账户维度
             ...
             , COUNT(IF(trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, member_id,
                        NULL))        AS payment_num        --支付成功例子数
             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, interest,
                      0))             AS interest_num       --太极兴趣度例子数

             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, influence,
                      0))             AS influence_num      --太极紧迫度例子数

             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, sex,
                      0))             AS sex_num            --太极女性度例子数
             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, age,
                      0))             AS age_num            --太极50岁以上例子数
             ...
             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, is_old_user,
                        0))                                                                    AS olduser_num        -- 老用户例子数
             , SUM(IF(member_status = 1 AND trade_state IN ('SUCCESS', 'PREPARE') AND sales_id > 0, is_old_user_30,
                        0))                                                                    AS olduser_num30     -- 老用户例子数30
   ...
   , r1 AS
    (SELECT '${datebuf}'                                          AS d_date
          ...
          , goods_name
          , CASE
                WHEN GROUPING__ID = 255 THEN '1_海内外x期次x品类'
                WHEN GROUPING__ID = 63 THEN '2_海内外x期次x品类x投放部门x渠道'
                WHEN GROUPING__ID = 31 THEN '3_海内外x期次x品类x投放部门x渠道x版位'
                WHEN GROUPING__ID = 239 THEN '4_海内外x期次x品类x部门'
                WHEN GROUPING__ID = 231 THEN '5_海内外x期次x品类x部门x组'
                WHEN GROUPING__ID = 225 THEN '6_海内外x期次x品类x部门x组x销售'
                WHEN GROUPING__ID = 15 THEN '7_海内外x期次x品类x投放部门x渠道x版位X部门'
                WHEN GROUPING__ID = 7 THEN '8_海内外x期次x品类x投放部门x渠道x版位X部门X组'
                WHEN GROUPING__ID = 1 THEN '9_海内外x期次x品类x投放部门x渠道x版位X部门X组X销售'
                WHEN GROUPING__ID = 0 THEN '10_海内外x期次x品类x投放部门x渠道x版位X部门X组X销售X账户'
            END                                                   AS grouptype
          -- ,GROUPING__ID
          ...
          , SUM(wx_active_num)                                    AS wx_active_num      --20241126调整位置
          , SUM(collect_active_num)                               AS collect_active_num --20241126调整位置
          , SUM(payment_num)                                      AS payment_num        --支付成功例子数
          , SUM(interest_num)                                     AS interest_num       --太极兴趣度例子数
          , SUM(influence_num)                                    AS influence_num      --太极紧迫度例子数
          , SUM(sex_num)                                          AS sex_num            --太极女性例子数
          , SUM(age_num)                                          AS age_num            --太极50岁以上例子数
     ...
     GROUP BY is_abroad
         ...
         GROUPING SETS (
            ...
            , (is_abroad, goods_name, cat, department)                                                -- 00011101111
            , (is_abroad, goods_name, cat, department, user_group)                                    -- 00011100111
            , (is_abroad, goods_name, cat, department, user_group, sales_name, sales_sop_type)        -- 00011100001
            , (is_abroad, goods_name, cat, ad_department, platform_name, pos, department)             -- 00000001111
            , (is_abroad, goods_name, cat, ad_department, platform_name, pos, department, user_group) -- 00000000111
            , (is_abroad, goods_name, cat, ad_department, platform_name, pos, department, user_group, sales_name
            , sales_sop_type)                                                                         -- 00000000001
            , (is_abroad, goods_name, cat, ad_department, platform_name, pos, department, user_group, sales_name
            , sales_sop_type
            , cost_id)                                                                                -- 00000000000
         ))
...
OVERWRITE
TABLE
app.c_app_course_period_sale_deliver_dashboard
PARTITION
(
dt = '${datebuf}'
)
