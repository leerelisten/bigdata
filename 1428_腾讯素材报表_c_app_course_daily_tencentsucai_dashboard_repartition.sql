CREATE TABLE IF NOT EXISTS app.c_app_course_daily_tencentsucai_dashboard_repartition
(
    ...
    COMMENT '培训主题数仓-腾讯素材报表-重新分区'
    PARTITIONED BY (dt STRING)
    ROW FORMAT DELIMITED
        FIELDS TERMINATED BY '\001'
    STORED AS TEXTFILE
    LOCATION '/dw/app/c_app_course_daily_tencentsucai_dashboard_repartition';
...
INSERT OVERWRITE TABLE app.c_app_course_daily_tencentsucai_dashboard_repartition PARTITION (dt)
SELECT a.d_date
     , '日' as group_type
     , null as goods_name
     , a.cat
     ...
     , a.conversions_rate
     , a.conversions_cost
     , a.video_outer_play_count
     , a.video_outer_play_rate
     , a.video_outer_play_cost
     , a.video_outer_play_time_count
     , a.video_outer_play10_count
     , a.video_outer_play25_count
     , a.video_outer_play50_count
     , a.video_outer_play75_count
     , a.video_outer_play90_count
     , a.video_outer_play95_count
     , a.video_outer_play100_count
     , a.video_outer_play3s_count
     , a.video_outer_play5s_count
     , a.video_outer_play7s_count
...
FROM app.c_app_course_daily_tencentsucai_dashboard a
         LEFT JOIN (SELECT is_abroad
                    ...
                    WHERE dt = '${datebuf}'
                      AND report_id = '1428') pm
                   ON (pm.ad_department = '全部' OR a.ad_department = pm.ad_department)
WHERE a.dt = '${datebuf}'
  AND a.d_date >= '2024-09-27'
  AND a.platform_name IN ('腾讯')
  AND a.pos IN ('腾讯公众号', '腾讯朋友圈', '腾讯视频号信息流','腾讯视频号信息流(北京)','腾讯朋友圈(北京)')
GROUP BY a.d_date
       , a.cat
       , a.ad_department
       , a.platform_name
...
select a.d_date
      ...
      , CONCAT_WS(',', COLLECT_SET(pm.emails)) AS permission
      , a.d_date AS dt
FROM app.c_app_course_period_tencentsucai_dashboard a
         LEFT JOIN (SELECT is_abroad
                         , cat
                         , platform_name
                         , pos
