SET mapred.job.name="c_app_course_period_dyzbsucai_dashboard#${datebuf}";
USE app;
CREATE EXTERNAL TABLE IF NOT EXISTS app.c_app_course_period_dyzbsucai_dashboard
(
    ...
    COMMENT '培训主题数仓-抖音直播素材期次报表'
    PARTITIONED BY (dt STRING)
    ROW FORMAT DELIMITED
        FIELDS TERMINATED BY '\001'
    STORED AS TEXTFILE
    LOCATION '/dw/app/c_app_course_period_dyzbsucai_dashboard';
    ...
    (SELECT
     ...
     WHERE a.dt = '${datebuf}'
       AND TO_DATE(CONCAT('20', SUBSTR(a.goods_name, 2, 2), '-', SUBSTR(a.goods_name, 4, 2), '-',
                          SUBSTR(a.goods_name, 6, 2))) >= '2025-05-22'
       AND a.created_at >= '2025-05-20'
...
INSERT
    OVERWRITE
    TABLE
    app.c_app_course_period_dyzbsucai_dashboard
    PARTITION
(
    dt = '${datebuf}'
)
...
DFS -touchz /dw/app/c_app_course_period_dyzbsucai_dashboard/dt=${datebuf}/_SUCCESS;