...
CREATE TABLE IF NOT EXISTS dwd.dwd_erzhuan_performance_pay_order
(
    ...
    pay_date_compare_current_month   STRING COMMENT '支付日期比较本月',
    is_current_month_teacher         STRING COMMENT '班主任是否本月',
    is_before_month_teacher          STRING COMMENT '班主任是否上月统计过',
    pay_date_compare                 STRING COMMENT '支付日期比较3月31日',
...
from  dwd.dwd_edu_service_payment_orders
where   substr(pay_time,1,10) between '2025-03-01'  and '2025-04-30'
...
SELECT a.order_period_name
     ...
     , SUBSTR(a.pay_time, 1, 10)                                        AS pay_date                         --支付时间
     ...
     , IF(SUBSTR(a.pay_time, 1, 10) >= IF(d.teacher IS NOT NULL, d.start_date, '9999-01-01') AND
          SUBSTR(a.pay_time, 1, 10) <= IF(d.teacher IS NOT NULL, d.end_date, '0000-01-01'), 1,
          0)                                                            AS pay_date_compare_current_month   --支付时间比较本月
     ...
     , IF(SUBSTR(a.pay_time, 1, 10) > '2025-03-31', 1, 0)               AS pay_date_compare                 --支付日期和上月底做比较  注：每个月需调整
...
--数据插入底表
INSERT OVERWRITE TABLE dwd.dwd_erzhuan_performance_pay_order PARTITION (date_month = '2025-04')
SELECT order_period_name
     ...
     , pay_date_compare_current_month                                                                                             --支付时间比较本月
     , is_current_month_teacher                                                                                                   --是否当月班主任
     , is_before_month_teacher                                                                                                    --班主任是否上个月统计过
     , pay_date_compare                                                                                                           --支付日期和上月底做比较  注：每个月需调整
     , if_tongji                                                                                                                  --是否应该统计
...
FROM (
         SELECT order_period_name
              ...
              , IF(is_current_month_classer <> 0, 1,
                   IF(is_current_month_teacher <> 0 AND pay_date_compare_current_month <> 0,
                      IF(pay_date_compare <> 0, 1, IF(is_before_month_teacher = 0, 1, 0)),
