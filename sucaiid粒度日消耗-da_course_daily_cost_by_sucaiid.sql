SET mapred.job.name="da_course_daily_cost_by_sucaiid#${datebuf}";
USE da;
CREATE EXTERNAL TABLE IF NOT EXISTS da.da_course_daily_cost_by_sucaiid
(
    ...
    COMMENT '培训主题数仓-投放属性-素材id粒度消耗数据'
    PARTITIONED BY (dt STRING)
    STORED AS ORC
    LOCATION '/dw/da/da_course_daily_cost_by_sucaiid';
...
WITH base AS
         (SELECT t1.cdate              AS dt
               ...
               , t1.material_id        AS sucaiid
               , t1.material_name      AS sucainame
          ...
          FROM (SELECT cdate         --抓取日期
                     ...
                     , json_data
                FROM ods.ods_marketing_report_material --api抓取的原始数据
...
OVERWRITE
TABLE
da.da_course_daily_cost_by_sucaiid
PARTITION
(
dt = '${datebuf}'
)
